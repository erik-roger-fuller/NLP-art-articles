...
def article_processor(folder_path, chunksize, begin):
...
for begin in range(0, len(filelist), chunksize): # 5 len(filelist)
    per = time.time()
    df_all = article_processor(folder_path, chunksize, begin)

    #df_all.to_pickle(f"pkl_backups/second_pass/new_pass_first_corpus_{begin}_to_{begin+chunksize}.pkl")
    df_all = df_all.reset_index()
    df_all.to_feather(f"pkl_backups/second_pass/new_pass_first_corpus_{begin}_to_{begin+chunksize}.feather")
